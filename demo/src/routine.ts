import { BREAK_PATHS, DIMS, apCost, capOf, doAction, doTrain, preAct, preLog, pushEvent, rankReq, render, soloSkill } from "./main";
import { questNote } from "./quest";
import { SQUAD_ACTS, doSquad } from "./squad";
import { S } from "./state";
...
/* 记录一次行动 */
export function noteAct(kind, key){
...
export function actLabel(a){
  ...
  if(a.k === "squad"){
    const s = SQUAD_ACTS.find(x => x.k === a.v);
    return s ? s.n : "战队";
  }
  if(a.k === "pre"){
...
export function actListText(list){
  if(!list || !list.length) return "空";
  const c = {};
  list.forEach(a => { const n = actLabel(a); c[n] = (c[n] || 0) + 1; });
  return Object.entries<number>(c).map(([n, v]) => v > 1 ? `${n}×${v}` : n).join(" · ");
}
...
export function runActs(list, fromCursor?){
  ...
  for(; i < list.length; i++){
    ...
    if(a.k === "train"){
      ...
      const canSquad=S.squad&&(!inPre()||(S.pre&&S.pre.mates&&S.pre.mates.length));
...
export function clearPlan(){ S.plan = null; render(); }
...
export function quickPlanPre(){
  const P=S.pre; if(!P) return;
  const cost=k=>apCost(k);
  ...
  if(done){
    const txt=Object.entries<number>(acts).map(([n,v])=>v>1?`${n}×${v}`:n).join("、");

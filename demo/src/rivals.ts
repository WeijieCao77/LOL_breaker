...
import { POSN, SEASONS, SPLITS, clamp, pushEvent } from "./main";
import { S } from "./state";
...
export function rivalKey(p){ return p.id; }
export function ensureRivals(){ if(!S.rivals) S.rivals={}; }

/* 记仇：weight 越大越难忘 */
export function noteGrudge(teamName,weight,reason){
  ensureRivals();
...
/* 复仇：赢回来 */
export function noteRevenge(teamName){
  ensureRivals();
  const t=findTeam(teamName); if(!t) return;
  const foe=t.players.find(p=>p.pos===S.pos)||t.players[0];
  if(!foe) return;
  ...
  if(r.beat>=r.lost&&true) checkAch("revenge");   // 审计：钩子缺失
}
export function topRival(){
  ensureRivals();
  const arr=Object.values(S.rivals).filter((r: any)=>r.declared).sort((a: any,b: any)=>b.heat-a.heat);
  return arr[0]||null;
}
/* 宿敌在场时，你会更投入（心态高的人享受这个，心态低的人被压住） */
export function rivalBoost(oppTeam){
  const r: any=topRival();
...
export function rivalCard(){
  ...
  return `<div class="card"><h2>宿敌<em>${r.beat}胜 ${r.lost}负</em></h2>
    <h3>${avatarOf(r,30)} ${r.id}${r.cn?`（${r.cn}）`:""} <span class="tag">${POSN[r.pos]}</span>

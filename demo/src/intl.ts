...
export function worldsSlot(playerResult){
...
/* ---------- 组建世界赛 16 强 ---------- */
export function buildWorldsField(playerResult,cfg){
...
/* ---------- 入围赛：8 队双败，取前 4 ---------- */
export function simPlayIn(field){
...
/* ---------- 事件入口 ---------- */
export function startIntl(type,playerResult){
  const HL=S.homeLeague||"LPL";
...
/* 抽签名单：按赛区列出来，你自己的队标粗 */
export function intlDrawText(title,field){
  const byLg={};
...
export function brInit(field,double){
...
export function brSim(p){ return !p[0]?p[1]||null:!p[1]?p[0]:simBo(p[0],p[1],3); }
export function brStep(){
...
export function brApply(st,winners){
  const B=S.intl.br;
...
/* 你不在场时，把整个赛事跑完 */
export function simWholeEvent(field,stage){
...
export function simEventStaged(field,stage){
...
/* ---------- 世界线张力（2026-09-03 玩家拍板：均衡档）----------
   ...
   首年进王朝队，全部由同一个场自然处理。 */
export function wlOf(lg){ return (S.wl&&S.wl[lg])||0; }
export function wlAdd(lg,amt){ if(!lg||!amt) return; S.wl=S.wl||{}; S.wl[lg]=clamp((S.wl[lg]||0)+amt,0,1); }
export function wlInfluence(){
...
export function convergeChamp(type,field,simmed){
...
export function convergeStaged(type,field,st){
...
export function canonQual(playin,qual){
  try{
...
export function spectateIntl(type){
...
/* 生涯最深战绩：数字越大越深 */
export const INTL_DEPTH={"main":1,"knock":2,"semi":3,"final":4,"champion":5};
export function noteDepth(kind){

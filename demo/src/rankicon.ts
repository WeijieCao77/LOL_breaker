...
/* 小段罗马数字 */
export const ROMAN={"一":"I","二":"II","三":"III","四":"IV"};

/* 段位 -> 官方徽章文件名 */
export function tierArtKey(v){
...
export function rankIcon(v,size){
  size=size||44;
...
/* 徽章 + 文字 */
export function rankBadge(v,size){
...
export function teamLogo(name,size){
  size=size||22;
  if(!name) return "";
  // 职业前 S.world 还不存在，所以要按 世界 -> offer 世界 -> 原始数据 依次找
  const srcs=[];
  if(S){ if(S.world)srcs.push(S.world); if(S.pre&&S.pre.world)srcs.push(S.pre.world); }
  if(DATA.leagues) srcs.push(DATA.leagues);

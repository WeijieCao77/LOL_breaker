...
import { squadOf } from "./squad";
import { S } from "./state";
import { avgTrust } from "./team";
import { checkTryoutInvite } from "./tryout";
...
/* 对手强度随轮次递增 */
export function cupOf(k){ return (S.cups||{})[k]; }
export function activeCups(): any[]{ return Object.values<any>(S.cups||{}).filter((c: any)=>c.alive); }
export function rollOpp(){ return (rnd()-0.5)*3.0; }   // 每轮抽一次，签运
export function cupOppPower(k){
...
export function powerRank(p){ return tierOf(p); }   // 统一实力尺：同一张表
export function cupOppName(k){
  const c=cupOf(k), C=CUPS[k];
...
export function cupPersonal(p){ return strength(p); }
export function cupMyPower(k){
  ...
  if(S.pre&&S.pre.mates&&S.pre.mates.length){
    ...
    const syn=squadOf("syn");
    const tac=squadOf("tac");
    const mor=avgTrust();
...
export function cupPrep(k){
  const c=cupOf(k);
  const cost=apCost("cupprep");
...
/* 本周到点、但还没打的比赛 */
export function dueCups(){
  return activeCups().filter((c: any)=>c.nextWeek<=S.pre.week);
}
/* 不打就是弃权。
   原来推进周次完全不检查这个：比赛可以被无限期挂着，赛程也不往前走，
   于是「本周开打」永远显示在那儿，等于这场比赛不存在。
   现实里没上场就是弃权，这里也一样——但必须先问清楚。 */
export function forfeitCup(k){
  const c=cupOf(k), C=CUPS[k];
  if(!c||!c.alive) return;
  c.alive=false;
...
export function endCupMatch(){
  ...
  try{
    const soloNow=cupPersonal(S.attrs)+(gearBonus("操作")*0.4);
    ...
    m.snap={ round:c.round, prep:c.prep, my:cupMyPower(k), teamName:cupTeamName(),
      ...
      syn:squadOf("syn"), tac:squadOf("tac"),
      mor:avgTrust(), legacyPrep:(S.pre&&S.pre.cupPrep)||0 };
  }catch(e){}
  // 打得越多，被俱乐部看到的次数越多。累积在这里，但兑现要等整届打完——
  // 赛程进行中发邀请会把比赛线整个短路（试过：玩家半决赛就签约走人）。
  S.pre.scoutSeen = (S.pre.scoutSeen||0) + 1;
  tacAdd(2, `${C.name}${cupRoundName(k,c.round)}`);   // 成建制比赛每打一轮都涨
  if(won){
    c.wins++;
    addFans(4+c.round*2);
...
export function cupPayout(k,champion){
  ...
  checkAch("cup",{kind:k,win:reached});
  // 关键：不用夺冠。走得远，数据被记下来，就有人来问。
  if(k!=="show"&&true) checkTryoutInvite(k,reached,champion);
  if(champion&&k!=="show") preLog(`<b>${C.name} 冠军。</b>这个名字开始有人记住了。`,"big");
  if(champion&&k!=="show"&&true) tacAdd(3,`${C.name}夺冠`);
...
export function cupDismissMatch(){
  const m=S.cupMatch;
  if(m&&m.result) S.cupResult=m.result;
  S.cupMatch=null;
  if(m&&m.disbandAfter&&true) disbandCrew();
  render();
}
  ...
  return list.map((c: any)=>{
    ...
    return `<div class="card cup"><h2>${C.name}<em>${cupRoundName(k,c.round)} · 第 ${c.round}/${C.rounds} 轮</em></h2>
      ...
      ${S.pre.mates&&S.pre.mates.length?`<div class="oppinfo">${
        S.pre.mates.map(m=>`<span class="chipx">${avatarOf(m,18)} ${m.id} <b>${POSN[m.pos]}</b><i class="ovr">实力 ${
          (ovrOf(m)).toFixed(0)}</i></span>`).join("")}</div>
...
export function cupPostCard(m){
  ...
  if(hasMates){
    ...
    const syn=sn?sn.syn:(squadOf("syn"));
    const tac=sn?sn.tac:(squadOf("tac"));
    const mor=sn?sn.mor:(avgTrust());
  ...
  if(fails.length){
    ...
    const worst=Object.entries<number>(cnt).sort((a,b)=>b[1]-a[1])[0][0];
    ...
    <div class="attrs">
      ${[{id:meName(),pos:S.pos,r:S.attrs,me:true}].concat(S.pre.mates).map(p=>{
        ...
        return `<div class="at wide"><div class="lb">${POSN[p.pos]}</div>${bar(v)}
          <div class="vn mono"><b>${v.toFixed(0)}</b>${avatarOf(p,22)}<span class="pname">${

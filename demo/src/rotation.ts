...
/* ---------- 一、赛程记录 ---------- */
export function fixKey(){ return `${S.si}-${S.split||0}`; }
export function fixNote(week,opp,sc,won,played){
...
export function fixtureCard(){
  ...
  const trs=rows.map(x=>{
    ...
    return `<tr class="${now?'me':''}" style="${x.r?'':'opacity:.8'}">
      <td class="n">${x.w}</td><td>${teamLogo(x.opp,18)}${x.opp}</td>
...
export function setBreakAgenda(items,head){
  if(!S.off) return;
  S.off.agenda=items; S.off.agendaHead=head||"";
}
export function breakAgendaCard(){
...
/* ---------- 三、训练赛对位挑战 ---------- */
export const SCRIM_EDGE_NEED=3;       // 攒到这个数，教练给一场正赛试用
export const SCRIM_POOL=[
...
export function scrimState(){ if(!S.scrim) S.scrim={edge:0,wins:0,played:0,trial:null,live:null}; return S.scrim; }
export function scrimCanStart(){
...
export function scrimWeekKey(d){
  d=d||0;
...
export function scrimOptP(opt){
  const inc=S.understudy; if(!inc) return 0.5;
  const mine=S.attrs[opt.dim]+(gearBonus(opt.dim));
...
export function scrimNode(){ const L=scrimState().live; if(!L) return null; return SCRIM_POOL[L.pool[(L.round-1)%L.pool.length]]; }
export function scrimPick(i){
...
export function scrimPostHtml(L,inc){
  if(!L||!L.done) return "";
...
export function scrimPanel(){
  ...
  return `<div class="scrimbox">
    ...
    <div class="row" style="margin-top:8px;align-items:center;gap:10px;flex-wrap:wrap">
      <button class="btn sm" id="scrimStart" ${c.ok?"":'disabled style="opacity:.45"'}>训练赛对位挑战 ${apTag("duel")}</button>
...
/* ---------- 四、正赛试用 ---------- */
export function scrimTrialActive(){ return !!(S.scrim&&S.scrim.trial); }
export function meAsPlayer(){ return {id:S.name||"你",cn:"",pos:S.pos,age:S.age,r:S.attrs,me:true}; }
export function grantTrial(games,why,opts?){
...
export function scrimTrialCheck(){
...
export function confirmStarter(why){
...
export function pickReplacement(){
  ...
  const r=makeRookie(S.pos,tavg-2,S.homeLeague||"LPL");
  return r||{id:"替补",pos:S.pos,age:20,r:Object.assign({},S.attrs)};
}
export function demoteCheck(won,gap){
...
export function rotationAfterMatch(won,gap){
  demoteCheck(won,gap);
...
export function mateInjuryTick(){
...
export function addRingTitle(t){ if(!S.career) return; S.career.ringTitles=(S.career.ringTitles||[]).concat([t]); }
export function ringTitles(){ return (S.career&&S.career.ringTitles)||[]; }

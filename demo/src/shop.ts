...
import { S } from "./state";
import { traitMul } from "./trait";
import { cerRecMul } from "./cer";
...
export const LEDGER_IN =[["salary","工资"],["match","比赛收入"],["stream","直播"],
...
export function addMoney(kind,n){
  n=Math.round(n||0);
  if(!n) return 0;
...
export function ledgerRotate(){
  if(!S.ledger) { initLedger(); return; }
  S.ledger.prev=S.ledger.cur;
  S.ledger.prevLabel=S.ledger.label;
  S.ledger.cur=_emptyLedger();
  S.ledger.label=nowLabel();
}
export function _ledgerSum(o: any): number{ return Object.values<number>(o||{}).reduce((a,b)=>a+b,0); }
...
export const TIER_N=["入门","进阶","职业","定制"];
...
/* 卡面写的必须是你真正拿到的数（玩家实锤 2026-09-10：「专业理疗写 +34 实际 +30」「短途度假写 +60 实际 +53」）。
   体力走 addFat：劳模 ×0.88、出征仪式的恢复倍率都在结算时才乘；信任走 addTrust：粘合剂 ×1.35 / 刺头 ×0.80。
...
export function gearOf(k){ return GEAR[k][(S.gear&&S.gear[k])||0]; }
/* 外设加成：直接加在属性发挥上（不提高天赋瓶颈，只是让你打出应有的水平） */
export function gearBonus(dim){
  let v=0;
...
/* 课程对训练的加成 */
export function courseTrainMul(dim){
...
export const PLATFORMS=["虎牙","斗鱼","B 站","快手"];
/* 一家俱乐部跟哪个平台合作。按队名定死——同一局里不会变，换队才会变。 */
export function clubPlatform(name?){
...
/* 来抢人的那家（一定不是俱乐部合作的那家） */
export function rivalPlatform(club){
...
export function doContent(k){
  const C=CONTENT_KINDS.find(x=>x.k===k);
  if(!C||S.ap<(apCost("content"))){ S.contentPick=null; render(); return; }
  S.contentPick=null; S.ap-=apCost("content");
...
export function streamFansMul(){ return 1.0; }
/* 平台推流：独家合同带来的涨粉加成，乘进 fanWeek 的收敛速率 */
export function streamPushMul(){
...
export function bizNote(kind,who,decision,money,note){
  try{
    S.bizLog=S.bizLog||[];
    S.bizLog.push({s:nowStamp(),
...
export function checkStreamBiz(){
  const f=S.fans||0;
  // 分成上调：一档一档来，每次都告诉玩家
  ...
  if(!S.streamDeal){
    while(idx<STREAM_CUTS.length-1&&f>=STREAM_CUTS[idx+1].at){
      ...
      if(S.pre&&!S.career&&true) preLog(evt,"good");
...
export function signStreamDeal(kind?){
...
export function declineStreamDeal(){
  const o=S.streamOffer; if(!o) return;
  S.streamOffer=null;
...
export function streamClauseCheck(){
  ...
  d.done=0;
  addMoney("other",-Math.min(fine,Math.max(0,S.money)));
...
export function streamOfferCard(){
  ...
  return `<div class="rankup"><div class="ru-inner" style="max-width:600px;text-align:left;max-height:86vh;overflow-y:auto">
    <div class="ru-icon" style="text-align:center">${gicon("stream",52)}</div>
...
export function streamDealCard(){
...
export function buyGear(slot,tier){
  ...
  checkAch("gear");   // 审计：装备到位的钩子缺失
  render();
}
export function buyCourse(k){
  ...
  checkAch("course");   // 审计：语言/全修成就的钩子缺失
  render();
}

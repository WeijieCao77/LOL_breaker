...
export const CHANGELOG=[
  ...
  {v:"v20260917a", at:"2026-09-17", items:[
    ...
    "<b>S6 出道的几条规则</b>（作者定）：LPL 2016 春 / 夏、2017 春打升降级，你的队卷进去就真的会降到 LSPL；2017 年 LSPL 夏季赛前二能拿 2018 年联盟席位；真实历史里新签进来的同位置选手照样进队当你的竞争者，他进队那年上半年你还是首发，之后他比你强一截就换他；2022 年之后世界继续变强，老将篇越打越难。"
  ]},
  ...
  {v:"v20260914b", at:"2026-09-14", items:[
    "<b>选手姓名更正</b>（玩家实锤：「Viper 的名字错了」「中国选手 Knight 的名字是韩文」）：导出数据时是按游戏 ID 查本名的，同一个 ID 有好几个人就取了第一条——Knight 有卓定、韩国的 이건、越南和拉美选手，Viper 有朴到贤、中国的何皓、泰国选手，四十多个 ID 配到了别人头上（Bin、Light、Ming、Angel、Hope、Cube、Puff、Ghost、River……）。现在按每个人自己的选手页面认人：Knight 是卓定；一批欧美、越南、巴西、土耳其选手身上被安的中文 / 韩文 / 俄文 / 希腊文名字清掉了（他们本来就没有中韩文名）。老存档读档时自动改过来",
    "<b>知名韩国选手显示中文名</b>：2022–2026 年打过 MSI / 世界赛的韩国选手里，有中文百科或赛事媒体出处的 69 人换成中文名（Faker 李相赫、Viper 朴到贤、Chovy 郑志勋、Keria 柳岷析、Ruler 朴载赫、ShowMaker 许秀、Canyon 金建敷……），找不到出处的 28 人保留韩文原名，不按读音硬译"
  ]},
  ...
  {v:"v20260911b", at:"2026-09-11", items:[
    ...
    "<b>你改写了什么</b>：结局页对照真实历史，逐届写出本作的冠军和真实冠军哪里不一样、你顶掉了谁。新出现的战队补齐了队标",
    "<b>卫冕被研究</b>（难度上调）：本赛季和上赛季你每拿一座冠军，对手打你时战力 <b>+1.5</b>，最多 +4——一座冠军都没拿过的人不受影响，拿得越多越被研究，专压连冠。比分页、下一场、备战页在对手战力旁边用红字写「卫冕压力 +X」。<b>老存档也生效</b>",
    "<b>对手战力的显示和判定对上了</b>：下一场、备战页、本周对手、赛程表原来显示的是对手带默契战术的整队战力，比赛里判定用的却是五个人的实力加权——现在显示的就是判定用的那个数，所以这几处的对手战力会比原来低一截：不是对手变弱了，是原来多写了"
  ]},
  ...
  {v:"v20260909w", at:"2026-09-09", items:[
    "<b>生涯名片的冠军区重做</b>（玩家实锤：「世界赛的夺冠明明是更重要的事情，但是在名片里重点被放在了下面的联赛成绩」）：原来所有冠军拿逗号串成一行、同字号同颜色，「S16 世界赛」和「S14 PCS夏季赛」一个分量；而占版面最大的是下面那张逐年常规赛名次表（八年能吃掉 896px）——<b>最重的那座奖杯被最轻的信息压住了</b>。现在分三档：<b>世界赛 46px 金色带年份独占一行</b>、MSI 次之、联赛压成灰字小号只报座数；一座国际冠军都没有时联赛自动升为主角，不会变成一片灰。逐年表行高上限 112→84，省下的高度还给冠军区"
  ]},
  ...
  {v:"v20260908k", at:"2026-09-08", items:[
    "实力条画错（作者截图：自己 83、队友 83，自己的条长一截）：带头像名字的行右列原来按内容自适应，「你」一个字比队友的名字窄，中间那根条就长出来一截——同一个数画得不一样长。右列改成固定宽（手机也固定），队伍页的五人实力、队友信任、试训里「你 vs 一队」这些行现在同样的数就是同样的长度；只有数值的行（首发竞争、信任、俱乐部关注、轮换资本）右列单独收窄，条不白让位置"
  ]},
  ...
  {v:"v20260907f", at:"2026-09-07", items:[
    "【测试中】五年之后可以再打三年：S16 收官页多了「再打三年 / 退役」，之后每年年末都能选退役，S19 年末强制退役；自由身年末桌上没报价的只能退役。年纪大了恢复变慢，伤病风险按年龄加",
    "结局层级：连续三年世界冠军才叫「王朝」，三连再加 MSI 和 26 岁以后的冠军是「传奇」；原来两座世界冠军的「王朝」改叫「两冠」。新成就：三连 S 冠、再战、老骥伏枥、大器晚成、最后一舞",
    "生涯名片按实际打了几年排；页头标语与结局文案不再写死「五年」"
  ]},
  {v:"v20260907e", at:"2026-09-07", items:[
    "职业前一年从 20 周压到 16 周（玩家实锤「培养冗长」）：每周行动点 12→10，每一点的收获相应变多，训练还顺带练第二维（操作带体质、运营带指挥……）；进队时会提示「职业训练更细、越往上越难」。城市争霸赛、主播杯、全明星、转会窗口按新周数重排。正在职业前的老档读取时按比例换算周数，已签约的档不受影响",
    "职业前多了 16 件日常事（网吧联赛、直播间黑粉、家里催找工作、假经纪人、期末考、车队内讧、观众送外设、大主播连麦……），空周更不空"
  ]},
  {v:"v20260907d", at:"2026-09-07", items:[
    "界面可以换成浅色或米色了：顶栏最右边「深 / 浅 / 米」随时切，手机上是一颗循环键，「教程说明」栏目和建档页右上角也有。两套浅底按白底重新调过色，按钮、结局名片、页头主视觉照旧是深的。选择记在这台设备上，不进存档"
  ]},
...
/* 支持作者（爱发电）：填上主页链接后，会出现右下角 ♥、页脚入口；自动提示只在生涯结束的结局页弹一次（每个浏览器会话最多一次）；
...
export function acceptOffer(i){
  ...
  if(!back) checkAch("sign",{first:true});   // 重返职业不是「第一份合同」——野路子那类成就不该再发（玩家实锤）；first：「职业前」那一栏只在这一次判（achieve.ts 的 preCareerOpen）
...
export function render(){
  ...
  if(S.step==="end"&&!S.cer) supportNudge("end");   // 只有生涯结束、退役仪式收起之后才提「支持作者」（作者定的：中途不弹）
  /* 大标题只在创建页和结局页铺开。

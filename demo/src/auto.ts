...
import { DIMS, WND_OPEN, fanFill, preLog, pushEvent, render } from "./main";
import { RANDOM_EVENTS, resolveRandom } from "./random";
import { weekKey } from "./routine";
import { safeName } from "./save";
...
export function autoOn(k){ return !!(S.auto && S.auto[k]); }
export function autoAnyOn(){ return AUTO_KEYS.some(x=>autoOn(x.k)); }
export function autoAllOn(){ return AUTO_KEYS.every(x=>autoOn(x.k)); }
export function autoSet(k,v){
  S.auto=S.auto||{};
  S.auto[k]=!!v;
  render();
}
export function autoToggleAll(){
  const on=!autoAllOn();
  S.auto=S.auto||{};
...
export function autoNote(txt){
  S.autoLog=S.autoLog||[];
  S.autoLog.push(txt);
  if(S.autoLog.length>40) S.autoLog=S.autoLog.slice(-40);
}
export function autoLogClear(){ S.autoLog=[]; }

/* ---------- 一、采购 ----------
   先留出安全余额：手上永远得有下一次报名费和几次理疗的钱，
   不然托管会把你花到打不起比赛。 */
export const AUTO_RESERVE=120;   // 经济重锚后水位整体下移，安全余额跟着降
export function autoCash(){ return (S.money||0)-AUTO_RESERVE; }

/* 外设槽位排序：先武装自己最强的那一维——托管不替玩家改打法，
   只把他已经在走的路走顺。 */
export function autoGearOrder(){
  const rank={};
...
export function autoStreamPick(){
  const fill=fanFill();
...
/* 事件的推荐项写在事件自己身上（rec 字段）。
   ...
   不能让托管把玩家花破产。 */
export function autoPickOption(list, rec){
...
export function autoDaily(){
  if(S.rndEv){
    const def=RANDOM_EVENTS.find(e=>e.id===S.rndEv.id);
...
/* ---------- 四、生涯（默认关，开的时候会警告） ---------- */
export function autoCareerStep(){
  const P=S.pre;
  ...
  if(P && P.invite && P.invite.pending){
    ...
    const gap=(tryoutSkill()-iv.expect);
    const wndOpen=P.week>=WND_OPEN;
  ...
  if(S.proOffer){
    const o=S.proOffer;
    const mine=clubStanding(S.team);
    const his=clubStanding(o.team);

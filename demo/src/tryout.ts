...
import { CUPS, activeCups, cupReachName } from "./cup";
import { myForm } from "./form";
import { DIMS, POSN, SEASONS, acceptOffer, addFat, avg, cap, clamp, cloneWorld, fanTier, leagueBaseline, leaveRoster, markTeamJoin, myRoster, myStrength, myTeam, power, preLog, preNextYear, pushEvent, pwShow, q1, rankFull, rankName, rankToSkill, rebuildMySchedule, render, strength, tacOf, teamCode } from "./main";
import { rnd } from "./rng";
...
export function tryoutSkill(){ return myStrength(); }
...
export function tierCard(){
  ...
  return `<div class="card"><h2>俱乐部门槛<em>你的综合 <b>${me.toFixed(0)}</b>${rk!==null&&true?` · ${rankFull(rk)}`:""}</em></h2>
...
export function checkTryoutInvite(kind, reached, champion){
  const C = CUPS[kind];
  if(!C || !S.pre) return;
  ...
  const why = `${C.name}${champion ? "夺冠" : (cupReachName(kind, reached))}`;
...
export function queueInvite(tier, reason, lg?){
...
export function checkRankInvite(){
  const P = S.pre; if(!P) return;
  flushInviteQ();   // 排队的先发
...
/* 「有没有人找你」和「能不能通过」是两件事。
   ...
   其余交给试训自己去筛。 */
export const TIER_ORDER = ["acad", "low", "mid", "top"];
export function fitTier(tier){
  const me = tryoutSkill();
...
export function canInvite(tierWanted){
  const P = S.pre; if(!P) return false;
  ...
  if(activeCups().length) return false;
  return true;
}
/* 人气到了也有人来问——直播这条路本来就是设计里的一条路 */
export function checkFanInvite(){
  const P = S.pre; if(!P) return;
  P.fanInvited = P.fanInvited || {};
  for(const [at, tier] of [[55,"acad"],[110,"low"],[190,"mid"]] as [number, string][]){
...
export function addInvite(tier, reason, lg?){
  const P = S.pre; if(!P) return;
...
export function clubStanding(name){
  try{
...
/* 按档次挑一家真实存在的俱乐部 */
export function pickClub(tier, league?){
  try{
...
export function inviteCard(){
  ...
  return `<div class="rankup"><div class="ru-inner" style="max-width:560px;text-align:left;max-height:86vh;overflow-y:auto">
    <div class="ru-icon" style="text-align:center">${gicon("scout",52)}</div>
    ...
    <div class="ru-tier" style="font-size:21px;text-align:center;margin-bottom:12px">${
      teamLogo(iv.team,22)} <b>${iv.team}</b>${(()=>{
...
export function tryoutDay(){ const t=S.tryout; return TRYOUT_DAYS[tryoutDays(t)[t.day]]; }
/* 体质差的人，后面几天会明显掉状态——这是体质在这里的意义 */
export function tryoutFatiguePenalty(){
  const t = S.tryout;
  const stamina = S.attrs.体质;
  return t.day * Math.max(0, (52 - stamina)) * 0.055;
}
export function resolveTryoutDay(i){
...
export function endTryout(){
  ...
  // 反复刷到 A+ 为止——那评级就白设计了。
  consumeOffer(t.team);
  checkAch("tryout", { grade:t.result.g });
  render();
}
...
export function dealLeverage(grade){
...
/* 把这家从今年的可选名单里划掉 */
export function consumeOffer(team){
  const P = S.pre; if(!P) return;
...
export function noteScoutInterest(){
  ...
  if(t && t !== S.team){
    ...
    if(rnd()<0.35) queueFollowUp("rumorSpread",3,{team:t});
  }
...
export function rollProOffers(wnd?){
  wnd = wnd || "year";
...
/* 小赛区不是背景板，是低谷时的退路。
   ...
   语言不通会影响默契。 */
export const MINOR_LEAGUES=["PCS","VCS","LJL","LLA","CBLOL","LCO","TCL"];
/* 是不是真的卡住了——这是「退路」该不该出现的判据 */
export function inRut(perf){
  return (S.benchedSplits||0)>=1 || (S.homeLeague||"LPL")==="LDL" || perf<3;
}
export function pickForeign(perf){
  const here=S.homeLeague||"LPL";
  if(here!=="LPL"&&here!=="LDL") return null;          // 已经在外面了
  const opts=[];
  const kr = hasCourse("kr");
  const en = hasCourse("en");
  if(perf < 12){
...
export function proOfferCard(){
  ...
  return `<div class="rankup"><div class="ru-inner" style="max-width:560px;text-align:left;max-height:86vh;overflow-y:auto">
    <div class="ru-icon" style="text-align:center">${gicon("transfer",52)}</div>
    <div class="ru-eyebrow" style="text-align:center">${o.asked?"有人接了你的牌":"转会问询"} · ${T.n}</div>
    <div class="ru-tier" style="font-size:21px;text-align:center;margin-bottom:12px">${
      teamLogo(o.team,22)} <b>${o.team}</b>${
    ...
    <p class="note" style="margin:0 0 10px">
      ...
      ${(o.league&&o.league!=="LPL")?`<br><b style="color:var(--gold)">这是 ${o.league}。</b>${
        (o.league==="LCK"&&hasCourse("kr"))?"你会韩语，沟通不是问题。"
        :(o.league!=="LCK"&&hasCourse("en"))?"你会英语，沟通不是问题。"
...
export function faRollOffers(wnd){
  const perf=proPerf();
...
export function faCard(){
  ...
  return `<div class="card" style="border-left:3px solid var(--red)"><h2>你是自由身<em>${wnd?`${txWindowName()} · 还有 ${left} 周关闭`:"注册窗还没开"}</em></h2>
    ...
    ${list.length?`<div class="grid g2">${list.map((o,i)=>`<button class="act" data-fa="${i}" ${(S.proOffer||S.tryout||S.deal)?'disabled style="opacity:.5"':''}>
        <div class="t">${teamLogo(o.team,18)}${o.team} <span class="tag ${o.tier==="top"?"g":""}">${CLUB_TIERS[o.tier].n}${o.league&&o.league!=="LPL"?" · "+o.league:""}</span></div>
...
export function renewCard(){
  ...
  if(!txWindowOpen()) return "";   // MSI / 世界赛还在打：合同还没到期，窗口没开
...
export function renewNegotiate(){
  const r=S.pendingRenew; if(!r||S.deal) return;
  const perf=proPerf();
  const coach=coachTrust();
...
export function contractLeftText(){
...
export function txWindowName(){
  return S.off && S.off.next==="summer" ? "季中注册窗" : "年底注册窗";
}
export function canAskTransfer(){
...
export function transferBudget(tier, grade){
...
export function makeProDeal(tier, team, grade, league){
...
export function signTransfer(){
  ...
  leaveRoster(old, oldLg);   // 旧队补人，别留幻影
  S.team = d.team;
  ...
  markTeamJoin();   // 换了队，在队时长归零
  S.trust = {}; initTrust();
  syncTrust();
  initRelations();   // 新东家：队友关系从头建，别挂一排默认 50
  ...
  disruptSynergy(1, `<b>${meName()}</b> 转会加盟`);
...
export function promoteRaiseOdds(){
...
export function askPromoteRaise(){
...
export function declinePromote(){
  const d = S.promoteDeal; if(!d) return;
  S.promoteDeal = null;
  addStaff("mgr",-3);
...
export function promoteDealCard(){
  const d = S.promoteDeal; if(!d) return "";
  const p = promoteRaiseOdds();
  const arc = (S.archive||[]).filter(x=>x.si===S.si);
  ...
  return `<div class="rankup"><div class="ru-inner" style="max-width:560px;text-align:left">
    ...
    <div class="ru-tier" style="font-size:21px;text-align:center;margin-bottom:12px">${
      teamLogo(d.team,22)} <b>${d.team}</b></div>
...
export function doSendDown(acad){
  const old=S.team;
  leaveRoster(old, S.homeLeague||"LPL");   // 一队名单里若有我，补人
...
export function promoteCard(){
  ...
  const me=myStrength(), him=inc?(strength(inc)):0;   // 统一实力尺：含装备，和队伍页同一个数
  const g=S.record?(S.record.w+S.record.l):0, wr=g>=3?(S.record.w/g):null;
  const ok1=me>=him+1;
  return `<div class="card"><h2>升队通道<em>母队 ${teamLogo(pname,18)} ${pname}</em></h2>
    <p class="note" style="margin:0 0 10px">合同签给的是俱乐部，你现在注册在二队名单。
      升队窗口：<b>季中间歇</b> 和 <b>休赛期</b>——到点自动核查，达标就调上去。<br>
      <span style="color:var(--ink-3)">口径：五维均值（含装备），和队伍页、天梯同一个数。LPL 均值约 ${Math.round((S.baseline&&S.baseline.LPL)||70)}，国服前 100 的门 ${rankToSkill(86).toFixed(0)}，前 10 的门 ${rankToSkill(95).toFixed(0)}，明星 85+。</span></p>
    <div class="attrs">
      ...
      <div class="at wide"><div class="lb">一队${POSN[S.pos]}</div>
        ...
        <div class="vn mono"><b>${him.toFixed(1)}</b>${inc?(avatarOf(inc,20))+`<span class="pname">${inc.id.slice(0,9)}</span>`:"—"}</div></div>
    </div>
...
/* ---------- 主动接触：点名一支队自荐 ---------- */
export function approachKey(){ return S.si + "-" + (S.split||0); }
...
export function approachLeagues(){
...
export function approachTargets(lgPick?){
...
export function transferPage(){
  ...
  const intentRows = intents.length ? intents.map(x=>`<tr>
      <td>${teamLogo(x.team,18)}${x.team}${x.asked?'<span class="tag">你去接触的</span>':""}</td>
...
export function selfRecOdds(tier){
...
export function preTransferPage(){
  const P=S.pre; if(!P) return "";
  const expo=exposureScore();
  const ladder: [number, string][]=[[45,"青训教练看得到你"],[110,"中游俱乐部看得到你"],[190,"豪门看得到你"]];
  const expCard=`<div class="card"><h2>圈内关注度<em>${Math.round(expo)}</em></h2>
    ...
    <p class="note">关注度＝名气 + 杯赛走多深 + 打过多少场有人看的比赛。
      周报报不报你、哪一档俱乐部来找你，都看这个数；段位（${rankFull(P.rank)}）是另一把尺子——值不值得看。</p></div>`;
  const c=canSelfRec();

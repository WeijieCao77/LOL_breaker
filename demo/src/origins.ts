import { DIMS } from "./main";
import { rnd } from "./rng";
import { COURSES } from "./shop";
...
export function bgOf(k){ return BACKGROUNDS.find(x=>x.k===k)||BACKGROUNDS[0]; }

/* 背景卡文案 · 文字版（2026-09-06 玩家点名：建档页不要详细数字）：只说方向和量级 */
export function bgEffectsWords(b){
  const out=[];

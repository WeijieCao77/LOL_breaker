...
export const ACH_MORE=[
  /* ---------- 职业前任务链（2026-09-04 玩家点名：前期要有会影响后面的事）---------- */
  {id:"road2stage", n:"从网吧到主舞台", d:"城市争霸赛进八强、拿到主播杯请柬、再赢下一座冠军。", tag:"职业前",
   on:"cup", cond:()=>!!(S.flags&&S.flags.chainCity&&S.flags.chainInv&&S.flags.chainChamp), r:{money:150,fame:40}},
  ...
  /* ---------- 再战三年（2026-09-07）---------- */
  {id:"threepeat", n:"三连 S 冠", d:"连续三年世界冠军。王朝。", tag:"荣誉",
   on:"crown", cond:()=>{ const ys=S.career.worldsYears||[]; return ys.includes(S.si)&&ys.includes(S.si-1)&&ys.includes(S.si-2); }, r:{money:5000,fame:500,fat:-99}},
  {id:"encore", n:"再战", d:"五年到了，你选择再打三年。", tag:"生涯",
   on:"extend", cond:()=>true, r:{fame:60}},
  {id:"oldguard", n:"老骥伏枥", d:"二十六岁以后拿下国际冠军。", tag:"荣誉",
   on:"crown", cond:()=>S.age>=26&&(((S.career.worldsYears||[]).includes(S.si))||((S.career.msiYears||[]).includes(S.si))), r:{money:1500,fame:200}},
  ...
  {id:"trust80", n:"更衣室之王", d:"全队对你的信任平均超过 80。", tag:"人际",
   on:"splitend", cond:()=>avgTrust()>=80, r:{money:260,fame:40}},
  {id:"nocrack", n:"没有裂痕", d:"队友之间任意两人的关系都在 60 以上。", tag:"人际",
   on:"splitend", cond:()=>{const v=Object.values<number>(S.rel||{});return v.length>=3&&Math.min(...v)>=60;},
  ...
  {id:"soul", n:"队魂", d:"威望达到 78。", tag:"人际",
   on:"splitend", cond:()=>cloutOf()>=78, r:{money:400,fame:80}},
  ...
  {id:"slumpwin", n:"低谷里的胜利", d:"状态跌到低谷时，仍然赢下比赛。", tag:"逆境",
   on:"win", cond:()=>myForm()<=34, r:{money:200,fame:45}},
  ...
  {id:"lowtrust", n:"众叛亲离还赢了", d:"全队信任低于 35 的情况下赢下比赛。", tag:"逆境",
   on:"win", cond:()=>avgTrust()<35, r:{fame:50}},
  ...
  {id:"allcourse", n:"全都学了", d:"把所有课程都修完。", tag:"经济",
   ...
   on:"course", cond:()=>COURSES.every(c=>S.courses&&S.courses[c.k]),
   r:{money:200,fame:40}},
  ...
  {id:"star1", n:"第一滴血", d:"第一次在正式比赛里赢下带明星选手的队伍。", tag:"宿敌",
   on:"match", cond:()=>Object.keys(S.beatStars||{}).length>=1, r:{fame:30}},
  {id:"beatfaker", n:"破神者", d:"三次战胜 Faker 所在的队伍。", tag:"宿敌",
   on:"match", cond:()=>beatCount("Faker")>=3, r:{fame:200,trust:10},
   flavor:"至暗时刻的墙上有一块最硬的砖。你把它敲下来了三次。"},
  {id:"beatchovy", n:"中路答卷", d:"三次战胜 Chovy 所在的队伍。", tag:"宿敌",
   on:"match", cond:()=>beatCount("Chovy")>=3, r:{fame:130}},
  {id:"beatknight", n:"内战无敌手", d:"三次战胜 Knight 所在的队伍。", tag:"宿敌",
   on:"match", cond:()=>beatCount("Knight")>=3, r:{fame:90}},
  ...
  {id:"asset4", n:"这就是生活", d:"四件资产全部入手。", tag:"经济",
   on:"asset", cond:()=>ASSETS.every(a=>(S.assets||{})[a.k]), r:{fame:80},
  ...
  {id:"reactgold", n:"手快", d:"决赛入场或试训上机的反应挑战拿到金档。", tag:"仪式",
   on:"cer", cond:(c)=>c&&(c.k==="final"||c.k==="bench")&&c.tier==="gold", r:{fame:12}},
  {id:"decidegold", n:"读秒决策", d:"版本发布会的五道题至少答对四题。", tag:"仪式",
   on:"cer", cond:(c)=>c&&c.k==="patch"&&c.tier==="gold", r:{fame:12}},
  {id:"allstargold", n:"技巧赛之王", d:"全明星技巧赛拿到金档。", tag:"仪式",
   on:"cer", cond:(c)=>c&&c.k==="allstar"&&c.tier==="gold", r:{fame:15}},
  {id:"trialpass", n:"这一关过了", d:"突破试炼过关——教练把你留下的那一晚。", tag:"仪式",
   on:"cer", cond:(c)=>c&&c.k==="trial"&&c.tier==="gold", r:{fame:10}},
  {id:"mediabold", n:"标题党", d:"媒体日三个回答全是狂话。", tag:"仪式",
   on:"cer", cond:(c)=>c&&c.k==="media"&&c.picks&&c.picks.length===3&&c.picks.every(x=>x==="bold"), r:{fame:8}},

...
import { S } from "./state";
import { addTrust, addTrustAll, avgTrust } from "./team";
...
export const SQUAD_DECAY = 0.21;       // 每赛段向 50 回归——这是个要一直喂的池子
...
export function disruptSynergy(changed,who){
  if(!S.squad) return;
...
export function squadBase(players){
  ...
  players.forEach(p=>{
    ...
    const ih=(!p.me&&true)?mateInjuryHit(p):0;
...
export function squadWeights(players,fatigue,team){
  ...
  return [
    ...
    {k:"mor", n:"士气", v:mine&&true?avgTrust():50,
     mult:1+(((mine&&true)?avgTrust():50)-50)/760},   // 与 trustMod 同步（减半）
...
/* ---------- 综合实力与差距 ---------- */
export function teamPowerOf(name){
...
export function clampWinProb(p,diff){
  if(diff<=-GAP_WINDOW){
...
export function squadCard(){
  ...
  return `<div class="card"><h2>战队战力<em>全队 ${pwShow(B.total).toFixed(1)}</em></h2>
    ...
    <div class="attrs">${mates.map(p=>{
      const ih=(!p.me&&true)?mateInjuryHit(p)*PW_SHOW:0;
...
export function squadActs(){
  ...
  return `<h3 style="font-size:13px;color:var(--ink-3);margin:16px 0 8px">战队${
    (!S.career&&S.pre&&true)?`<span class="tag g">${cupTeamName()}</span>`:""}</h3>

...
export const BRK: Record<string, BM[]> = {
  /* ---- S6 开档（2016–2021） ---- */
  /* 守擂赛：积分低的先打，胜者依次挑战种子更高的队（地区决赛）；3 队 / 4 队 */
  KOTH3: [M("a", "s2", "s3", 1, 5, 3), M("f", "s1", "W:a", 2, 5, 2, 1)],
  KOTH4: [M("a", "s3", "s4", 1, 5, 4), M("b", "s2", "W:a", 2, 5, 3), M("f", "s1", "W:b", 3, 5, 2, 1)],
  /* LCK 2016–2020 阶梯赛：4v5（BO3）→ 胜者打 3 → 胜者打 2 → 决赛打 1 */
  KOTH5: [M("a", "s4", "s5", 1, 3, 5), M("b", "s3", "W:a", 2, 5, 4), M("c", "s2", "W:b", 3, 5, 3), M("f", "s1", "W:c", 4, 5, 2, 1)],
  /* LPL 2016–2018：种子 A1 B1 A2 B2 A3 B3 A4 B4。组第 3 打异组第 4 → 胜者打异组第 2 → 半决赛打组第 1（跨组） */
  LPL8G: [M("r1a", "s5", "s8", 1, 5, 7), M("r1b", "s6", "s7", 1, 5, 7), M("qa", "s3", "W:r1b", 2, 5, 5), M("qb", "s4", "W:r1a", 2, 5, 5),
          M("sa", "s1", "W:qb", 3, 5, 3), M("sb", "s2", "W:qa", 3, 5, 3), M("f", "W:sa", "W:sb", 4, 5, 2, 1)],
  /* LPL 2019–2020：5v8、6v7 → 胜者打 4 / 3 → 半决赛打 1 / 2 */
  SE8B: [M("r1a", "s5", "s8", 1, 5, 7), M("r1b", "s6", "s7", 1, 5, 7), M("qa", "s4", "W:r1a", 2, 5, 5), M("qb", "s3", "W:r1b", 2, 5, 5),
         M("sa", "s1", "W:qa", 3, 5, 3), M("sb", "s2", "W:qb", 3, 5, 3), M("f", "W:sa", "W:sb", 4, 5, 2, 1)],

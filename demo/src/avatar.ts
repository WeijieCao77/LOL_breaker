...
export function _avaHash(s){
  let h=5381;
  for(let i=0;i<s.length;i++) h=(((h<<5)+h)^s.charCodeAt(i))>>>0;
  return h;
}

export const AVA_SKIN =["#EFC9A5","#E3B28A","#C99B72"];
export const AVA_HAIR =["#191A20","#2B2320","#503722","#8A6B3A","#B9BCC6","#7E3038"];
export const AVA_SHIRT=["#27455C","#5C2E38","#2F4A33","#4A3A5F","#5C4A27","#1F5058"];

/* 半身像：队服 + 脸 + 发型，可选眼镜和耳机。风格压平，跟界面一致。
   如果 data/photos/ 里放了这位选手的照片（经 make_avatars.py 烤成像素图），
   优先用像素头像——真人像素风，认得出是谁。 */
export function avatarOf(p,size){
  size=size||28;
  const id=((p&&(p.id||p))||"?")+"";
  if(AVATARS[id]){
...
export function gicon(name,size,color?){

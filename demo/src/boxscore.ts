...
import { POSN, POS_STATLINE, avg, clamp, myRoster, q1 } from "./main";
import { rnd } from "./rng";
...
export function boxAbility(p){
  // 「你」直读 S.attrs 并算上装备——名单里的拷贝读档后可能是旧的（玩家实锤：我的 61、队伍 53）
  const r=(p.me&&S&&S.attrs)?S.attrs:(p.r||p);
  const g=(p.me&&true)?(gearBonus("操作")*0.34+gearBonus("运营")*0.28+gearBonus("体质")*0.10):0;
  const v=(r.操作||50)*0.34+(r.运营||50)*0.28+(r.心态||50)*0.14+(r.体质||50)*0.10+g;
  const fm=(p.me?myFormMul():formMul(p));
  return v*fm;
}
export function synthBoxScore(m,won){
  ...
  const line=(p,side)=>{
    const T=(POS_STATLINE[p.pos])||[4.1,2.5,5.2,8.8,0.28];
...
/* ---------- 赛后 / 档案回放共用的表 ---------- */
export function boxScoreHtml(box,won,oppName){

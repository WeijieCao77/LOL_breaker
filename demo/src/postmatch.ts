...
export function attribute(myTeamPlayers,oppTeamObj,fatigue,verFav){
  ...
  const fixOf={
    默契:"换过人就要重新磨。多合练、多双排。",
    战术:"准备不够。战术复盘和训练赛。",
    士气:"更衣室出问题了。「队伍」栏里找人聊聊、约队友吃火锅、安排一次战队合练，都能把关系推回来。",
...
export function reviewAdvice(m){
  ...
  /* 2026-09-05 玩家点名：原来只列「比对位低」的行、长板赢的那份账不写，再配一句「没有短板惩罚」，
     读起来像只罚不奖。战力是加权和，长板加的和短板扣的是同一把秤——两边都列，最后给净账。 */
  const strong=cmp.filter(x=>x.g>=1).sort((a,b)=>b.eff-a.eff).slice(0,2);
  ...
  const extreme=sd>=9;
  const topW=strong.length?W[strong[0].d]:0;
  const net=cmp.reduce((a,x)=>a+x.eff,0);
  strong.forEach(({d,g,eff},i)=>{
    personal.push({good:true,q:`${d}比对位高 ${g.toFixed(0)} 分（权重 ${W[d]}）→ <b>+${pwShow(eff).toFixed(1)}</b> 战力`,
      how:i===0?`这是你的本钱——临场节点里多选吃「${d}」的选项，成功率直接看这一维；训练赛对位也用它打`:`长板的每一分和短板的每一分是同一把秤：高就加、低就扣，权重一样`});
  });
  weak.forEach(({d,g,eff})=>{
    const capped=S.attrs[d]>=capOf(d)-0.05;
    const P=BREAK_PATHS[d];
    personal.push({q:`${d}比对位低 ${(-g).toFixed(0)} 分（权重 ${W[d]}）→ <b>${pwShow(eff).toFixed(1)}</b> 战力`,
      how:capped&&P?`已到瓶颈——突破方法：${P.how}`
        :(extreme&&topW&&W[d]<topW)?`这项权重 ${W[d]} 比你长项（${strong[0].d} ${topW}）低：补 5 点值 ${pwShow(5*W[d]).toFixed(1)} 战力，长项练 5 点值 ${pwShow(5*topW).toFixed(1)}——把长项练到顶更划算`
        :(extreme&&topW&&W[d]>=topW)?`这项权重 ${W[d]} 不比你的长项低：补 5 点值 ${pwShow(5*W[d]).toFixed(1)} 战力，和练长项一样划算甚至更划算——别被「极端加点」四个字骗了`
        :`日常多点「练${d}」，装备/课程里也有加${d}的`});
  });
  if(strong.length||weak.length) personal.push({good:net>=0,q:`对位账：合计 <b>${net>=0?"+":""}${pwShow(net).toFixed(1)}</b> 战力（操作 .34 / 运营 .28 / 心态 .14 / 体质 .10；指挥走全队乘数另算）`,
    how:extreme?"极端加点的总账就是这一行：长项赢的减短项输的。另外临场节点抽到短板维度会吃亏——那种节点选另一个选项":"高就加、低就扣，没有额外的短板惩罚，也没有额外的长板奖励"});
  ...
  if(fd.length>=2){
    ...
    const worst=Object.entries<number>(c).sort((a,b)=>b[1]-a[1])[0][0];
  ...
  if(bad("默契")){
    let extra="";
    if(S.rel){
      const v=Object.entries<number>(S.rel).sort((a,b)=>a[1]-b[1])[0];
  ...
  if(bad("士气")) team.push({q:`更衣室不稳（平均信任 ${Math.round(avgTrust())}）`,
    how:"商城「团队建设」「约队友吃火锅」，更衣室事件别火上浇油；赢比赛本身也是最好的粘合剂"});
...
export function pmNodesHtml(nodes,luck){
...
export function pmAdviceHtml(adv,won,rows){
  const li=x=>`<div class="rv-i ${x.good?'good':''}"><span class="rq">${x.q}</span><span class="rh">${x.how}</span></div>`;

...
export const TRAIT_AXES={ hard:"硬", warm:"暖", grind:"苦", show:"秀" };
export const TRAIT_NEED=5;          // 同一条轴选够几次，就成了那样的人
/* 互为反面的两组：你不可能既是刺头又是老好人，也不可能既埋头又天天上镜。
   先成型的那个把对面挡掉——性格是排他的，不是可以集齐的收藏品。 */
export const TRAIT_OPP={ edge:"glue", glue:"edge", grinder:"star", star:"grinder" };
...
export function traitHas(k){ return !!(S.traits && S.traits.indexOf(k)>=0); }
export function traitList(){ return (S.traits||[]).map(k=>TRAITS.find(t=>t.k===k)).filter(Boolean); }
/* 把所有已有特质在某一项上的效果乘起来。没有特质就是 1（或 0，加法项） */
export function traitMul(key, add?){
  let v = add ? 0 : 1;
  traitList().forEach(t=>{
    const x=t.e[key];

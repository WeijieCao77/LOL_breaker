...
function questSettleDone(){
  if(!S.quests) return;
  const done=S.quests.filter(q=>q.prog>=q.need);
  if(!done.length) return;
  S.quests=S.quests.filter(q=>q.prog<q.need);
  done.forEach(q=>{
    if(q.money&&typeof addMoney==="function") addMoney("other",q.money);
    if(q.fans&&typeof addFans==="function") addFans(q.fans);
    if(q.trust&&typeof addTrustAll==="function") addTrustAll(q.trust);
    if(q.mgr&&typeof addStaff==="function") addStaff("mgr",q.mgr);
...
function questWeek(){
  ...
  dead.forEach(q=>{
    if(q.penalty){
      ...
      if(typeof addMoney==="function") addMoney("other",-pay);
    }
    if(q.penFans&&typeof addFans==="function") addFans(-q.penFans);
    if(q.mgr&&typeof addStaff==="function") addStaff("mgr",-Math.abs(q.mgr));
...
function evMakeRival(teamName,reason){
  if(typeof noteGrudge!=="function") return null;
...
/* 某支队真的换人：把一个老将换成新秀，队伍战力跟着变。
   ...
   否则新闻说了一遍，世界里什么也没发生。 */
function evRosterChange(team){
  if(!team||!team.players||typeof makeRookie!=="function") return null;
  const lg=(typeof leagueOf==="function")?leagueOf(team.name):"LPL";
  const base=(S.baseline&&S.baseline[lg])||50;
  let idx=-1, worst=1e9;
  team.players.forEach((p,i)=>{
    if(p.me) return;
    const v=(typeof ovrOf==="function")?ovrOf(p):50;
...
function evClubMood(mgr,coach){
  if(typeof addStaff!=="function") return;
